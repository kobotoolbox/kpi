...
(function() {
  ...
  var rootUrl = (function() {
    try {
      return document.head
        .querySelector('meta[name=kpi-root-url]')
        .content.replace(/\/$/, '');
    } catch (e) {
  ...
  assign(this, {
    ...
    logout: () => {
      var d = new $.Deferred();
      $ajax({ url: `${rootUrl}/api-auth/logout/` })
        .done(d.resolve)
    ...
    listAllAssets() {
      var d = new $.Deferred();
      $.when(
        $.getJSON(`${rootUrl}/assets/?parent=`),
        $.getJSON(`${rootUrl}/collections/?parent=`)
      )
        .done(function(assetR, collectionR) {
          var assets = assetR[0],
    ...
    assignPerm(creds) {
      ...
      var objectUrl =
        creds.objectUrl || `${rootUrl}/${creds.kind}s/${creds.uid}/`;
      var userUrl = `${rootUrl}/users/${creds.username}/`;
      var codename = creds.role.includes('_submissions')
        ? creds.role
        : `${creds.role}_${creds.kind}`;
      return $ajax({
      ...
      if (sort.length)
        s =
          sort[0].desc === true
            ? `&sort={"${sort[0].id}":-1}`
            : `&sort={"${sort[0].id}":1}`;
      if (fields.length) f = `&fields=${JSON.stringify(fields)}`;
      if (count) filter += '&count=1';
    ...
    login: creds => {
      return $ajax({
        url: `${rootUrl}/api-auth/login/?next=/me/`,
        data: creds,
        method: 'POST'
      });
    }
