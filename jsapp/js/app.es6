...
import { AddToLibrary, FormPage, LibraryPage } from './components/formEditors';
...
function stringifyRoutes(contextRouter) {
  return JSON.stringify(
    contextRouter.getCurrentRoutes().map(function(r) {
      return {
        name: r.name,
        href: r.path
      };
...
class App extends React.Component {
  ...
  componentWillReceiveProps() {
    ...
    if (this.state.pageState.modal) stores.pageState.hideModal();
  }
  componentDidMount() {
      ...
      <DocumentTitle title="KoBoToolbox">
        <div className="mdl-wrapper">
          ...
          >
            ...
            {!this.isFormBuilder() &&
              !this.state.pageState.drawerHidden && <Drawer />}
            <bem.PageWrapper__content
              className="mdl-layout__content"
              m={this.isFormSingle() ? 'form-landing' : ''}
            >
              {!this.isFormBuilder() && (
