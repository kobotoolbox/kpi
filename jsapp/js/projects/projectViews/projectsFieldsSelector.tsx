...
import { HOME_DEFAULT_VISIBLE_FIELDS, PROJECT_FIELDS } from './constants'
...
export default function ProjectsFieldsSelector(props: ProjectsFieldsSelectorProps) {
  const getInitialSelectedFields = () => {
    ...
    if (!props.selectedFields || props.selectedFields.length === 0) {
      outcome = HOME_DEFAULT_VISIBLE_FIELDS
    } else {
      outcome = Array.from(props.selectedFields)
    }
    return outcome.filter((fieldName) => !props.excludedFields?.includes(fieldName))
  }
  ...
  return (
    <div className={styles.root}>
      {/* Trigger button */}
      <Button type='text' size='s' onClick={toggleModal} startIcon='spreadsheet' label={t('fields')} />
      ...
      <KoboModal isOpen={isModalOpen} onRequestClose={toggleModal} size='medium'>
        ...
        <KoboModalFooter>
          <Button type='secondary-danger' size='m' onClick={resetFields} label={t('Reset')} />

          <Button type='primary' size='m' onClick={applyFields} label={t('Apply')} />
        </KoboModalFooter>
      </KoboModal>
    </div>
  )
