...
import {EnvStoreFieldItem} from 'js/envStore'
import {
  AssetTypeName,
  ValidationStatus,
  AssetFileType
...
export interface ImportResponse {
  ...
  url: string
  messages?: {
    updated?: {uid: string, kind: string, summary: AssetSummary, owner__username: string}[]
    created?: {uid: string, kind: string, summary: AssetSummary, owner__username: string}[]
    error?: string
    error_type?: string
  }
  status: 'processing' | 'complete' | 'created' | 'error'
}
...
export interface Permission {
  ...
  partial_permissions?: {
    url: string
    filters: {_submitted_by: {$in: string[]}}[]
  }[]
}
...
export const dataInterface: DataInterface = {
  ...
  bulkSetAssetPermissions(assetUid: string, perms: {user: string, permission: string}[]) {
    return $ajax({
