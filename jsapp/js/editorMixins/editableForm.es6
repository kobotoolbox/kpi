...
import { AVAILABLE_FORM_STYLES, update_states } from '../constants';
...
var webformStylesSupportUrl =
  'http://help.kobotoolbox.org/creating-forms/formbuilder/using-alternative-enketo-web-form-styles';
...
class FormSettingsBox extends React.Component {
  ...
  updateState(newState = {}) {
    'start end today deviceid'
      .split(' ')
      .forEach(this.passValueIntoObj('meta', newState));
    'username simserial subscriberid phonenumber'
      .split(' ')
      .map(this.passValueIntoObj('phoneMeta', newState));
  ...
  render() {
    var metaData = [...this.state.meta, ...this.state.phoneMeta]
...
export default assign(
  {
    componentDidMount() {
      ...
      document
        .querySelector('.page-wrapper__content')
    ...
    saveForm(evt) {
      ...
      if (this.state.editorState === 'new') {
        ...
        actions.resources.createResource.triggerAsync(params).then(asset => {
        ...
        actions.resources.updateAsset
          ...
          .catch(resp => {
            ...
            if (resp.statusText != 'error') errorMsg = resp.statusText;
      ...
      return (
        <bem.FormBuilderHeader>
          <bem.FormBuilderHeader__row
            m={['first', allButtonsDisabled ? 'disabled' : null]}
          >
            ...
            <bem.FormBuilderHeader__cell m={'buttonsTopRight'}>
              ...
              <bem.FormBuilderHeader__button
                m={[
                  ...
                  {
                    savepending:
                      this.state.asset_updated === update_states.PENDING_UPDATE,
                    savecomplete:
                      this.state.asset_updated === update_states.UP_TO_DATE,
                    savefailed:
                      this.state.asset_updated === update_states.SAVE_FAILED,
                    saveneeded:
                      this.state.asset_updated === update_states.UNSAVED_CHANGES
                  }
                ]}
                onClick={this.saveForm}
                className="disabled"
                disabled={
                  !this.state.surveyAppRendered || !!this.state.surveyLoadError
                }
              >
                <i />
                {saveButtonText}
      ...
      return (
        <DocumentTitle title={`${docTitle} | KoboToolbox`}>
          <ui.Panel m={'transparent'}>
            ...
            <bem.FormBuilder
              m={
                this.state.formStylePanelDisplayed ? 'formStyleDisplayed' : null
              }
            >
