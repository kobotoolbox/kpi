...
import { PlanNames, Product, SubscriptionChangeType } from 'js/account/stripe.types'
...
export const YourPlan = () => {
  ...
  const subscriptionUpdate = useMemo(() => getSubscriptionChangeDetails(currentPlan, productsContext.products), [currentPlan, productsContext.isLoaded])
  ...
  return (
    <article>
      <section className={styles.section}>
        <div className={styles.planInfo}>
          <p className={styles.plan}>
            <strong>{t('##plan_name## Plan').replace('##plan_name##', planName)}</strong>
          </p>
          <time dateTime={startDate} className={styles.start}>
            {t('Started on ##start_date##').replace('##start_date##', startDate)}
          </time>
          {subscriptionUpdate && (
            <Badge
              ...
              size={'s'}
              label={
                <time dateTime={subscriptionUpdate.date} className={styles.updateBadge}>
                  {subscriptionUpdate.type === SubscriptionChangeType.RENEWAL &&
                    t('Renews on ##renewal_date##').replace('##renewal_date##', formatDate(subscriptionUpdate.date))}
      ...
      {subscriptionUpdate?.type === SubscriptionChangeType.PRODUCT_CHANGE && (
        <div className={styles.subscriptionChangeNotice}>
          {t('Your ##current_plan## plan will change to the ##next_plan## plan on')
            .replace('##current_plan##', planName)
          ...
          {t('. You can continue using ##current_plan## plan features until the end of the billing period.').replace(
            '##current_plan##',
            planName,
          )}
        </div>
      ...
      {subscriptionUpdate?.type === SubscriptionChangeType.PRICE_CHANGE && (
        <div className={styles.subscriptionChangeNotice}>
          {t('Your ##current_plan## plan will change from annual to monthly starting from').replace(
            '##current_plan##',
            planName,
          )}
          &nbsp;
