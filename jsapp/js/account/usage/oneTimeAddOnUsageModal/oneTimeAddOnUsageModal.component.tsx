      ...
      <KoboModal isOpen={showModal} onRequestClose={toggleModal} size='medium'>
        ...
        <section className={styles.addonModalContent}>
          <div className={styles.addonUsageDetails}>
            <div className={styles.addonTypeHeader}>{t('##TYPE##').replace('##TYPE##', typeTitles[props.type])}</div>
            <ul className={styles.usageBreakdown}>
              <li>
                <label>
                  {t('Included with ##PERIOD## plan').replace('##PERIOD##', periodAdjectiveDisplay[props.period])}
                </label>
                <data>{limitDisplay(props.type, props.recurringLimit)}</data>
              </li>
              <li>
                <label>{t('From add-ons')}</label>
                <data>{limitDisplay(props.type, props.recurringLimit, props.remainingLimit)}</data>
              </li>
              <li>
                <label>{t('Used this ##PERIOD##').replace('##PERIOD##', props.period)}</label>
