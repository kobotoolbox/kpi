...
export default function OrgInviteAcceptedBanner({ username, organization }: OrgInviteAcceptedBannerProps) {
  const memberQuery = useOrganizationsMembersRetrieve(organization.id, username, {
    query: {
  ...
  useEffect(() => {
    if (memberQuery.data?.status === 200 && memberQuery.data?.data.invite.url) {
      ...
      setLocalStorageKeyPrefix(`kpiOrgInviteAcceptedBanner-${memberQuery.data?.data.invite.url}`)
    }
  }, [memberQuery.data?.status, (memberQuery.data?.data as MemberListResponse)?.invite?.url])
  ...
  if (
    ...
    memberQuery.data?.status !== 200 ||
    memberQuery.data?.data.invite?.status !== InviteStatusChoicesEnum.accepted ||
