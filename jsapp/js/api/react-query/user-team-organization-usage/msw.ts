...
export const getApiV2OrganizationsAssetsRetrieveResponseMock = (
  ...
  results: Array.from({ length: faker.number.int({ min: 1, max: 10 }) }, (_, i) => i + 1).map(() => ({
    ...
    project_ownership: {
      invite: faker.helpers.arrayElement([faker.internet.url(), undefined]),
      sender: faker.helpers.arrayElement([faker.string.alpha({ length: { min: 10, max: 20 } }), undefined]),
      recipient: faker.helpers.arrayElement([faker.string.alpha({ length: { min: 10, max: 20 } }), undefined]),
