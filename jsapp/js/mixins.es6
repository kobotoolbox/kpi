    ...
    if (params.base64Encoded) {
      stores.pageState.showModal({
        ...
        filename: multipleFiles
          ? t('## files').replace('##', params.totalFiles)
          : params.name
      });
    }
            ...
            .done(importData => {
              if (importData.status === 'complete') {
                ...
                if (multipleFiles) {
                  ...
                  if (!assetUid) {
                    ...
                    alertify.error(
                      t(
                        'XLSForm Import failed. Check that the XLSForm and/or the URL are valid, and try again using the "Replace with XLS" icon.'
                      )
                    );
                    if (params.assetUid)
              ...
              // If the import task didn't complete immediately, inform the user accordingly.
                ...
                if (importData.messages.error)
                  error_message = `<strong>${t(
                    'Import Failure.'
                  )}</strong><br><code><strong>${
                    importData.messages.error_type
                  }</strong><br>${importData.messages.error}</code>`;
  ...
  click: {
    asset: {
      ...
      edit: function(uid) {
        ...
        else hashHistory.push(`/forms/${uid}/edit`);
      },
      delete: function(uid) {
        ...
        if (!deployed) {
          ...
          else msg = t('You are about to permanently delete this draft.');
        } else {
          msg = `
            ${t('You are about to permanently delete this form.')}
            <div class="alertify-toggle"><input type="checkbox" id="dt1"/> <label for="dt1">${t(
              'All data gathered for this form will be deleted.'
            )}</label></div>
            <div class="alertify-toggle"><input type="checkbox" id="dt2"/> <label for="dt2">${t(
              'All questions created for this form will be deleted.'
            )}</label></div>
            <div class="alertify-toggle"><input type="checkbox" id="dt3"/> <label for="dt3">${t(
              'The form associated with this project will be deleted.'
            )}</label></div>
            <div class="alertify-toggle alertify-toggle-important"><input type="checkbox" id="dt4"/> <label for="dt4">${t(
      ...
      unarchive: function(assetOrUid) {
        let asset =
          typeof assetOrUid == 'object'
            ? assetOrUid
            : stores.selectedAsset.asset;
        let dialog = alertify.dialog('confirm');
        let opts = {
...
mixins.permissions = {
  ...
  userCan(permName, asset) {
    if (!asset.permissions) return false;

    if (!stores.session.currentAccount) return false;

    const currentUsername = stores.session.currentAccount.username;
    if (asset.owner__username === currentUsername) return true;
    ...
    const anonAccess = asset.permissions.some(
      perm =>
        perm.user__username === 'AnonymousUser' && perm.permission === permName
    );
    if (anonAccess) return true;

    const userPerms = asset.permissions.filter(
      perm => perm.user__username === currentUsername
    );
...
mixins.contextRouter = {
  isFormList() {
    return (
      this.context.router.isActive('forms') &&
      this.context.router.params.assetid == undefined
    );
  },
  isLibrary() {
  ...
  isFormBuilder() {
    if (this.context.router.isActive(`/library/new`)) return true;

    if (this.context.router.params.assetid == undefined) return false;

    var assetid = this.context.router.params.assetid;
    if (this.context.router.isActive(`/library/${assetid}/edit`)) return true;
