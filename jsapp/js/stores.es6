...
import { log, t, notify, assign } from './utils';
...
var assetLibraryStore = Reflux.createStore({
  init() {
    this.results = [];
    this.listenTo(
      actions.search.libraryDefaultQuery.completed,
      this.libraryDefaultDone
    );
  },
...
var historyStore = Reflux.createStore({
  ...
  historyPush(item) {
    this.history = [
      item,
      ...this.history.filter(function(xi) {
        return item.uid !== xi.uid;
      })
    ];
  ...
  routeUpdate(routes) {
    const routeName =
      routes.names[routes.names.length - 1] ||
      routes.names[routes.names.length - 2];
    if (this.currentRoute) this.previousRoute = this.currentRoute;
...
stores.snapshots = Reflux.createStore({
  init() {
    this.listenTo(
      actions.resources.createSnapshot.completed,
      this.snapshotCreated
    );
    this.listenTo(
      actions.resources.createSnapshot.failed,
      this.snapshotCreationFailed
    );
  },
...
var assetStore = Reflux.createStore({
  init: function() {
    ...
    this.listenTo(
      ...
      this.onLoadAssetCompleted
    );
    this.listenTo(
      actions.resources.updateAsset.completed,
      this.onUpdateAssetCompleted
    );
  },
  ...
  noteRelatedUsers: function(data) {
    ...
    data.permissions.forEach(function(perm) {
      ...
      var isOwnerOrAnon =
        username === data.owner__username || username === 'AnonymousUser';
  ...
  triggerLoggedIn(acct) {
    ...
    if (acct.dkobo_survey_drafts && acct.dkobo_survey_drafts.non_migrated) {
      ...
      let notify_content = `${wait_message}<br/>
        ...
        <small>
        (${acct.dkobo_survey_drafts.non_migrated}/${
        acct.dkobo_survey_drafts.total
      })
        </small>
      `;
      alertify.notify(
...
var assetContentStore = Reflux.createStore({
  init: function() {
    ...
    this.surveys = {};
    this.listenTo(
      actions.resources.loadAssetContent.completed,
...
var allAssetsStore = Reflux.createStore({
  init: function() {
    ...
    this._waitingOn = {};
    this.listenTo(
      actions.resources.listAssets.completed,
      this.onListAssetsCompleted
    );
    ...
    this.listenTo(
      actions.resources.createAsset.completed,
      this.onCreateAssetCompleted
    );
    this.listenTo(
      actions.resources.loadAsset.completed,
      this.loadAssetCompleted
    );
  },
  whenLoaded(uid, cb) {
...
var collectionAssetsStore = Reflux.createStore({
  init() {
    this.collections = {};
    this.listenTo(
      actions.resources.readCollection.completed,
      this.readCollectionCompleted
    );
  },
...
stores.collections = Reflux.createStore({
  init() {
    this.listenTo(
      actions.resources.listCollections.completed,
    ...
    this.listenTo(
      actions.resources.deleteCollection,
      this.deleteCollectionStarted
    );
    this.listenTo(
      actions.resources.deleteCollection.completed,
      this.deleteCollectionCompleted
    );
  },
...
var serverEnvironmentStore = Reflux.createStore({
  init() {
    this.state = {};
    this.listenTo(
      actions.misc.getServerEnvironment.completed,
...
if (window.Intercom) {
  var IntercomStore = Reflux.createStore({
    ...
    loggedIn(acct) {
      ...
      let legacyName = [acct.first_name, acct.last_name]
        .filter(val => val)
        .join(' ');
      let userData = {
        ...
        created_at: Math.floor(new Date(acct.date_joined).getTime() / 1000),
