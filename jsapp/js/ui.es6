  ...
  render() {
    return (
      <bem.uiPanel className={this.props.className} m={this.props.m}>
        <bem.uiPanel__body>{this.props.children}</bem.uiPanel__body>
      </bem.uiPanel>
    );
  }
}
  ...
  render() {
    return (
      ...
      >
        <div
          className={classNames(
            this.props.open ? 'modal-open' : 'modal',
    ...
    if (!name) {
      ...
      if (row_count) {
        if (row_count === 2) {
          ...
          extra = (
            <small>
              {t('and ## other questions').replace('##', row_count - 1)}
            </small>
          );
        }
      }
...
class PopoverMenu extends React.Component {
  ...
  toggle(evt) {
    ...
    if (this.state.popoverVisible || isBlur) {
      $popoverMenu = $(evt.target)
        .parents('.popover-menu')
        .find('.popover-menu__content');
      this.setState({
        popoverHiding: true
      });
    ...
    if (this.props.type == 'assetrow-menu' && !this.state.popoverVisible) {
      ...
      var $assetRowOffset = $(evt.target)
        .parents('.asset-row')
        .offset().top;
      var $assetListHeight = $(evt.target)
        .parents('.page-wrapper__content')
        .height();
    ...
    return (
      <bem.PopoverMenu m={[this.props.type, this.state.placement]}>
        <bem.PopoverMenu__toggle
          onClick={this.toggle}
          onBlur={this.toggle}
          data-tip={this.props.triggerTip}
          tabIndex="1"
        >
        ...
        <bem.PopoverMenu__content
          m={[
            this.state.popoverHiding ? 'hiding' : '',
            this.state.popoverVisible ? 'visible' : 'hidden'
          ]}
        >
          {this.props.children}
        </bem.PopoverMenu__content>
