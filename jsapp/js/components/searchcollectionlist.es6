...
class SearchCollectionList extends Reflux.Component {
  ...
  searchChanged(searchStoreState) {
    ...
    if (searchStoreState.searchState === 'done') this.queryCollections();
  }
  queryCollections() {
    ...
    return [
      ...
      <bem.AssetListSorts className="mdl-grid" key="2">
        <bem.AssetListSorts__item
          m={'name'}
          className="mdl-cell mdl-cell--8-col mdl-cell--4-col-tablet mdl-cell--2-col-phone"
        >
          {t('Name')}
        </bem.AssetListSorts__item>
        <bem.AssetListSorts__item
  ...
  renderGroupedHeadings() {
    return (
      <bem.AssetListSorts
        className="mdl-grid"
        style={{ width: this.state.fixedHeadingsWidth }}
      >
        <bem.AssetListSorts__item
          m={'name'}
          className="mdl-cell mdl-cell--5-col mdl-cell--4-col-tablet mdl-cell--2-col-phone"
        >
          {t('Name')}
        </bem.AssetListSorts__item>
        <bem.AssetListSorts__item
  ...
  renderGroupedResults() {
    ...
    var results = ['Deployed', 'Draft', 'Archived'].map((category, i) => {
      if (this.state[searchResultsBucket][category].length < 1) {
        return [];
      }
      return [
        <bem.List__subheading key={i}>{t(category)}</bem.List__subheading>,
