...
import { assign, t, formatTime, formatDate, stringToColor } from '../utils';
...
class FormSummary extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.chartPeriod != this.state.chartPeriod ||
  ...
  getLatestSubmissionTime(assetid) {
    ...
    dataInterface.getSubmissions(assetid, 1, 0, sort, fq).done(data => {
      if (data.length) this.setState({ lastSubmission: data[0]['end'] });
      else this.setState({ lastSubmission: false });
    });
  }
  renderSubmissionsGraph() {
    return (
      <bem.FormView__row m="summary-submissions">
        <bem.FormView__cell m="label">{t('Submissions')}</bem.FormView__cell>
        <bem.FormView__cell m={['box']}>
          <bem.FormView__cell m="subs-graph">
            <bem.FormView__cell m="subs-graph-toggle">
              <a
                onClick={this.showGraphWeek}
                className={this.state.chartPeriod == 'week' ? 'active' : ''}
              >
                {t('Past 7 days')}
              </a>
              <a
                onClick={this.showGraphMonth}
                className={this.state.chartPeriod == 'month' ? 'active' : ''}
              >
                {t('Past 31 days')}
              </a>
            </bem.FormView__cell>
            <bem.FormView__cell
              m={`summary-chart`}
              className={this.state.subsCurrentPeriod ? 'active' : 'inactive'}
            >
              <canvas
                ref="canvas"
                className={this.state.chartVisible ? 'visible' : ''}
              />
            </bem.FormView__cell>
          ...
          <bem.FormView__group m={['submission-stats']}>
            <bem.FormView__cell>
              <span className="subs-graph-number">
                {this.state.subsCurrentPeriod}
              </span>
              <bem.FormView__label>
                {this.state.chartPeriod == 'week' &&
            ...
            <bem.FormView__cell>
              <span className="subs-graph-number">
                {this.state.subsPreviousPeriod}
              </span>
              <bem.FormView__label>
                {this.state.chartPeriod == 'week' &&
                  `${formatDate(moment().subtract(7, 'days'))} - ${formatDate(
                    moment().subtract(13, 'days')
                  )}`}
                {this.state.chartPeriod != 'week' &&
                  `${formatDate(moment().subtract(31, 'days'))} - ${formatDate(
                    moment().subtract(60, 'days')
                  )}`}
              </bem.FormView__label>
            </bem.FormView__cell>
            <bem.FormView__cell>
              <span className="subs-graph-number">
                {this.state.deployment__submission_count}
              </span>
    ...
    const sideTabs = [
      {
        label: t('Reports'),
        icon: 'k-icon-report',
        path: `/forms/${this.state.uid}/data/report`
      },
      {
        label: t('Reports (legacy)'),
        icon: 'k-icon-report',
        path: `/forms/${this.state.uid}/data/report-legacy`,
        className: 'is-edge'
      },
      {
        label: t('Table'),
        icon: 'k-icon-table',
        path: `/forms/${this.state.uid}/data/table`
      },
      {
        label: t('Gallery'),
        icon: 'k-icon-photo-gallery',
        path: `/forms/${this.state.uid}/data/gallery`
      },
      {
        label: t('Downloads'),
        icon: 'k-icon-download',
        path: `/forms/${this.state.uid}/data/downloads`
      },
      {
        label: t('Map'),
        icon: 'k-icon-map-view',
        path: `/forms/${this.state.uid}/data/map`
      }
    ];
  ...
  renderTeam() {
    ...
    this.state.permissions.forEach(function(p) {
      if (
        p.user__username &&
        !team.includes(p.user__username) &&
        p.user__username != 'AnonymousUser'
      )
    ...
    return (
      <bem.FormView__row m="team">
        <bem.FormView__cell m="label">{t('Team members')}</bem.FormView__cell>
        {this.userCan('change_asset', this.state) && (
          ...
          <bem.FormView__column m="right">
              ...
              <bem.FormView__cell m={['box']}>
                <bem.FormView__group m={['items', 'summary-details-cols']}>
                  ...
                  {this.state.summary &&
                    ...
                    this.state.summary.languages.length > 1 && (
                      <bem.FormView__cell>
                        <bem.FormView__label>
                          {t('Languages')}
                        </bem.FormView__label>
            ...
            {this.state.deployment__submission_count > 0 && (
              <bem.FormView__row m="data-links">
                <bem.FormView__cell m="label">{t('Data')}</bem.FormView__cell>
                <bem.FormView__cell m="box">
                  {this.renderDataTabs()}
                </bem.FormView__cell>
              </bem.FormView__row>
