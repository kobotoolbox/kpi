...
class LibrarySidebar extends Reflux.Component {
  ...
  createCollection() {
    ...
    let opts = {
      ...
      message: t(
        'Please enter the name of your new Collection. Collections can help you better organize your library, and it is possible to share each collection with different people.'
      ),
  ...
  deleteCollection(evt) {
    ...
    let opts = {
      ...
      message: t(
        'are you sure you want to delete this collection? this action is not reversible'
      ),
      labels: { ok: t('Delete'), cancel: t('Cancel') },
      onok: (evt, val) => {
        dataInterface
          .deleteCollection({ uid: collectionUid })
          .then(data => {
            this.quietUpdateStore({
              parentUid: false,
              parentName: false,
  ...
  renameCollection(evt) {
    ...
    let opts = {
      ...
      onok: (evt, val) => {
        actions.resources.updateCollection
          .triggerAsync(collectionUid, { name: val })
          .then(data => {
            this.queryCollections();
            dialog.destroy();
          });
  ...
  setCollectionDiscoverability(discoverable, collection) {
    return evt => {
      ...
      if (permDeferred) {
        var discovDeferred = permDeferred
          .then(() => {
  ...
  render() {
    return (
      <bem.CollectionsWrapper>
        <ui.PopoverMenu type="new-menu" triggerLabel={t('new')}>
        ...
        {this.state.sidebarCollections && (
          <bem.FormSidebar>
                ...
                return (
                  ...
                  >
                    ...
                    {!this.state.filteredByPublicCollection &&
                      this.state.filteredCollectionUid === collection.uid && (
                        <ui.PopoverMenu
                          type="collectionSidebarPublic-menu"
                        ...
                        >
                          {collection.access_type === 'owned' &&
                            collection.discoverable_when_public && (
                              <bem.PopoverMenu__link
                                m={'make-private'}
                                onClick={this.setCollectionDiscoverability(
                                  false,
                                  collection
                                )}
                              >
            ...
            <bem.FormSidebar__grouping
              m={[
                this.state.publicCollectionsVisible ? 'visible' : 'collapsed'
              ]}
            >
