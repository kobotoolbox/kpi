...
class Submission extends React.Component {
    ...
    dataInterface
      ...
      .done(data => {
        ...
        if (this.props.ids && sid) {
          ...
          if (c > 0) prev = this.props.ids[c - 1];
          if (c < this.props.ids.length) next = this.props.ids[c + 1];
        }

        const survey = this.props.asset.content.survey;
        const betaQuestions = ['begin_kobomatrix'];
        const hasBetaQuestion =
          survey.find(q => betaQuestions.includes(q.type)) || false;
  ...
  deleteSubmission() {
    ...
    let opts = {
      ...
      message: `${t('Are you sure you want to delete this submission?')} ${t(
        'This action cannot be undone'
      )}.`,
      labels: { ok: t('Delete'), cancel: t('Cancel') },
      onok: (evt, val) => {
        dataInterface
          .deleteSubmission(this.props.asset.uid, this.props.sid)
          .done(data => {
  ...
  renderAttachment(filename, type) {
    ...
    attachmentUrl = `${kc_base}/attachment/original?media_file=${encodeURI(
      filename
    )}`;
  ...
  renderRows() {
    ...
    return survey.map(q => {
              ...
              {s[name] &&
                s[name].map((repQ, i) => {
                  ...
                  for (var pN in repQ) {
                    ...
                    const subQ = survey.find(
                      x => x.name == qName || x.$autoname == qName
                    );
                    ...
                    if (icon)
                      type = (
                        <i
                          className={`fa fa-${icon.attributes.faClass}`}
                          title={q.type}
                        />
                      );
      ...
      if (q.type === 'end_repeat') return false;
      ...
      if (parentGroup) name = `${parentGroup}/${name}`;

      if (q.label == undefined || s[name] == undefined) {
        return false;
      }
      ...
      if (icon)
        type = (
          <i className={`fa fa-${icon.attributes.faClass}`} title={q.type} />
        );
      ...
      return (
        <tr key={`row-${name}`}>
          <td className="submission--question-type">{type}</td>
          <td className="submission--question">
            {q.label[translationIndex] || t('Unlabelled')}
          </td>
      ...
      <bem.FormModal>
        {this.state.hasBetaQuestion && (
          <div className="submission--warning">
            ...
            <span>
              {t(
                'Responses from a Question Matrix are not displayed in this screen.'
              )}
            </span>
          </div>
        )}
          ...
          <div className="submission-actions">
            {this.userCan('change_submissions', this.props.asset) &&
              this.state.enketoEditLink && (
                <a
                  href={this.state.enketoEditLink}
