...
import { assign, t, log } from '../utils';

function labelVal(label, value) {
  return {
    label: t(label),
    value: value || label.toLowerCase().replace(/\W+/g, '_')
  };
}
...
class ChartColorsPicker extends React.Component {
  ...
  defaultReportColorsChange(e) {
    this.props.onChange(
      ...
      {
        report_colors:
          reportColorSets[e.currentTarget.value].colors ||
          reportColorSets[0].colors
      }
    );
  }
...
class QuestionGraphSettings extends React.Component {
  ...
  saveQS(reset) {
    ...
    if (!customReport) {
      ...
      report_custom[customReport.crid].specified[_qn] = reset
        ? {}
        : this.state.rStyle;
  ...
  questionStyleChange(params, value) {
    ...
    if (value && value.report_type) styles.report_type = value.report_type;
    if (value && value.report_colors)
      styles.report_colors = value.report_colors;
    if (params && params.id == 'width') styles.width = params.value;
      ...
      return (
        <button
          className={`mdl-button mdl-button--tab ${
            this.state.activeModalTab === i ? 'active' : ''
          }`}
            ...
            {this.state.activeModalTab === 1 && (
              <div id="graph-colors">
                ...
                <SizeSliderInput
                  ...
                  min="300"
                  max="900"
                  default={reportStyle.graphWidth}
                  label={t('Width: ')}
                  onChange={this.questionStyleChange}
                />
              </div>
  ...
  render() {
    ...
    for (var i = reportData.length - 1; i > -1; i--) {
      ...
      if (_type === 'select_one' || _type === 'select_multiple') {
        let question = asset.content.survey.find(
          z => z.name === _qn || z.$autoname === _qn
        );
        let resps = reportData[i].data.responses;
        if (resps) {
          reportData[i].data.responseLabels = [];
          for (var j = resps.length - 1; j >= 0; j--) {
            var choice = asset.content.choices.find(
              o =>
                question &&
            ...
            if (choice && choice.label && choice.label[tnslIndex])
              reportData[i].data.responseLabels.unshift(
                choice.label[tnslIndex]
              );
            else reportData[i].data.responseLabels.unshift(resps[j]);
          }
        } else {
          ...
          if (vals && vals[0] && vals[0][1] && vals[0][1].responses) {
            ...
            reportData[i].data.responseLabels = [];
            let qGB = asset.content.survey.find(
            ...
            respValues.forEach(function(r, ind) {
              ...
              reportData[i].data.responseLabels[ind] =
                choice && choice.label && choice.label[tnslIndex]
                  ? choice.label[tnslIndex]
                  : r;
            });
    ...
    return (
      <div>
        {reportData.map((rowContent, i) => {
          var label =
            rowContent.row.label && rowContent.row.label[tnslIndex]
              ? rowContent.row.label[tnslIndex]
              : t('Unlabeled');

          if (!rowContent.data.provided) return false;
                ...
                <div className="graph-tab__groupby" id="graph-labels">
                  <label htmlFor={'groupby-00'} key="00">
                    <input
                      ...
                      checked={
                        reportStyle.groupDataBy.length === 0 ? true : false
                      }
                      id={'groupby-00'}
                    />
                    {t('No grouping')}
                    ...
                    return (
                      <label htmlFor={'groupby-' + i} key={i}>
                        <input
                          ...
                          onChange={this.groupDataBy}
                          checked={
                            reportStyle.groupDataBy === val ? true : false
                          }
                          id={'groupby-' + i}
                        />
                        {translations
            ...
            {this.state.activeModalTab === 3 &&
              translations && (
                <div className="graph-tab__translation" id="graph-labels">
                  {this.props.parentState.asset.content.translations.map(
                    (row, i) => {
                      return (
...
class Reports extends React.Component {
  ...
  loadReportData() {
    ...
    stores.allAssets.whenLoaded(uid, asset => {
      ...
      if (asset.content.survey != undefined) {
        ...
        dataInterface
          .getReportData({ uid: uid, identifiers: names, group_by: groupBy })
          .done(data => {
            ...
            this.setState({
              ...
              translations:
                asset.content.translations.length > 1 ? true : false,
              groupBy: groupBy,
              error: false
            });
          })
    ...
    return (
      <bem.FormView__reportButtons>
        <ui.PopoverMenu
          ...
          triggerLabel={
            this.state.currentCustomReport
              ? this.state.currentCustomReport.name || t('Untitled Report')
              : t('Custom Reports')
          }
        >
        ...
        {this.state.currentCustomReport && (
          <button
            className="mdl-button mdl-button--icon report-button__edit"
        ...
        <button
          className="mdl-button mdl-button--icon report-button__print"
        ...
        {this.userCan('change_asset', this.state.asset) && (
          <button
            className="mdl-button mdl-button--icon report-button__settings"
  ...
  render() {
    ...
    if (asset && asset.content) docTitle = asset.name || t('Untitled');
