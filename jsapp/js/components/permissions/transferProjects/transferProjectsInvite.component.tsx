...
export default function TransferProjectsInvite(props: TransferProjectsInviteProps) {
  ...
  function decline() {
    ...
    declineInvite(props.inviteUid).then(() => {
      setIsDeclined(true)
      props.setInvite(TransferStatuses.Declined, asset?.assetName, asset?.assetOwner)
  ...
  function accept() {
    ...
    acceptInvite(props.inviteUid).then(() => {
      ...
      setIsModalOpen(!isModalOpen)
      props.setInvite(TransferStatuses.Accepted, asset?.assetName, asset?.assetOwner)
      setSearchParams()
    })
  }
  ...
  return (
    <KoboModal isOpen={isModalOpen} onRequestClose={toggleModal} size='medium'>
      <KoboModalHeader onRequestCloseByX={toggleModal} headerColor='white'>
        {isDeclined ? t('Project transfer declined') : 'Accept ownership transfer'}
      </KoboModalHeader>
      <div>
        <section className={styles.modalBody}>
          {isDeclined && (
            <p>
              {t('You have declined the request of transfer ownership for ##PROJECT_NAME##.').replace(
                '##PROJECT_NAME##',
          ...
          {!isDeclined && (
            <>
              ...
              <strong>{t('Once you accept, the transfer might take a few minutes to complete.')}</strong>
            </>
          )}
          <div
            className={cx({
