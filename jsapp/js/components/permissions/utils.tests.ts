import type { PartialPermission, PermissionResponse, PartialPermissionFilter } from '#/dataInterface'
import {
  hasPartialByUsers,
...
const PARTIAL_BY_USERS_FILTERS: PartialPermissionFilter[] = [{ _submitted_by: { $in: ['john', 'olivier'] } }]
...
const PARTIAL_BY_RESPONSES_FILTERS: PartialPermissionFilter[] = [{ Where_are_you_from: 'Poland' }]
...
describe('permissions utils', () => {
  ...
  describe('hasPartialByResponses', () => {
    it('should recognize a partial "by responses" permission', () => {
      chai.expect(hasPartialByResponses(GENERAL_PERM_BY_RESPONSES)).to.equal(true)
    })
    ...
    it('should recognize that non-partial permission is not "by users"', () => {
      chai.expect(hasPartialByResponses(GENERAL_PERM_NON_PARTIAL)).to.equal(false)
    })
  })

  describe('isPartialByUsersFilter', () => {
    it('should match partial "by users" filter', () => {
      chai.expect(isPartialByUsersFilter(PARTIAL_BY_USERS_FILTERS[0])).to.equal(true)
    })

    it('should not match other partial filter', () => {
      chai.expect(isPartialByUsersFilter(PARTIAL_BY_RESPONSES_FILTERS[0])).to.equal(false)
    })
  })

  describe('getPartialByUsersFilterList', () => {
    it('should find partial "by users" filter in filters with one', () => {
      chai.expect(getPartialByUsersFilterList(PARTIAL_PERM_BY_USERS)).to.deep.equal(['john', 'olivier'])
    })

    it('should not find partial "by users" filter in filters with single different filter', () => {
      chai.expect(getPartialByUsersFilterList(PARTIAL_PERM_BY_RESPONSES)).to.equal(undefined)
    })

    it('should find partial "by users" filter in filters with multiple AND filters', () => {
      chai.expect(getPartialByUsersFilterList(PARTIAL_PERM_BY_BOTH_AND)).to.deep.equal(['zoe', 'xavier'])
    })

    it('should find partial "by users" filter in filters with multiple OR filters', () => {
      chai.expect(getPartialByUsersFilterList(PARTIAL_PERM_BY_BOTH_OR)).to.deep.equal(['phil', 'vanessa'])
    })
  })

  describe('isPartialByResponsesFilter', () => {
    it('should match partial "by responses" filter', () => {
      chai.expect(isPartialByResponsesFilter(PARTIAL_BY_RESPONSES_FILTERS[0])).to.equal(true)
    })

    it('should not match other partial filter', () => {
      chai.expect(isPartialByResponsesFilter(PARTIAL_BY_USERS_FILTERS[0])).to.equal(false)
    })
  })
  ...
  describe('getPartialByResponsesFilter', () => {
    ...
    it('should not find partial "by responses" filter in filters with single different filter', () => {
      chai.expect(getPartialByResponsesFilter(PARTIAL_PERM_BY_USERS)).to.equal(undefined)
    })

    it('should find partial "by responses" filter in filters with multiple AND filters', () => {
      chai.expect(getPartialByResponsesFilter(PARTIAL_PERM_BY_BOTH_AND)).to.deep.equal({ What_do_you_love: 'Chaos' })
    })

    it('should find partial "by responses" filter in filters with multiple OR filters', () => {
      chai.expect(getPartialByResponsesFilter(PARTIAL_PERM_BY_BOTH_OR)).to.deep.equal({ Your_fav_animal: 'Racoon' })
    })
  })
})
