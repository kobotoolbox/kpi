...
import { formatTime, currentLang, assign, t, log, notify } from '../utils';
...
export class FormLanding extends React.Component {
    ...
    var available_links = new Map([
      ...
      [
        'single_url',
        {
          ...
          desc: t(
            'This allows a single submission, and can be paired with the "returnURL" parameter to redirect the user to a URL of your choice after the form has been submitted.'
          )
        }
      ],
      [
    ...
    return (
      <bem.FormView__row>
        <bem.FormView__cell m="columns">
          <bem.FormView__cell m="label">{t('Collect data')}</bem.FormView__cell>
        </bem.FormView__cell>
          ...
          <bem.FormView__cell m={['columns', 'padding']}>
            <bem.FormView__cell>
              <ui.PopoverMenu
                ...
                triggerLabel={available_links.get(chosenMethod).label}
              >
            ...
            <bem.FormView__cell>
              {chosenMethod != 'iframe_url' &&
                ...
                this.state.deployment__links[chosenMethod] && (
                  <CopyToClipboard
                    text={this.state.deployment__links[chosenMethod]}
              ...
              {chosenMethod != 'iframe_url' &&
                chosenMethod != 'android' && (
                  <a
                    className="collect-link mdl-button mdl-button--colored"
                    target="_blank"
          ...
          <bem.FormView__cell m={['padding', 'bordertop', 'collect-meta']}>
            ...
            {chosenMethod == 'android' && (
              <ol>
                ...
                <li>
                  {t('Click on')} <i className="fa fa-ellipsis-v" />{' '}
                  {t('to open settings.')}
                </li>
                <li>
  ...
  setCollectMethod(evt) {
    var method = $(evt.target)
      .parents('.popover-menu__link')
      .data('method');
    this.setState({
        ...
        <bem.FormView m="form">
          <bem.FormView__row>
            <bem.FormView__cell m={['columns', 'first']}>
              ...
              <bem.FormView__cell>{this.renderButtons()}</bem.FormView__cell>
            </bem.FormView__cell>
            <bem.FormView__cell m="box">
