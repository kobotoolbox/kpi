...
let newFormMixins = [Reflux.ListenerMixin, editableFormMixin];
...
var formViaUrlHelpLink =
  'http://help.kobotoolbox.org/creating-forms/importing-an-xlsform-via-url';
          ...
          {this.props.context != 'existingForm' && (
            <bem.FormModal__item m="upload-note">
              ...
              <label className="long">
                {t(
                  'Enter your project details below. In the next step, you can import an XLSForm (via upload or URL) or design the form from scratch in the Form Builder. '
                )}
              </label>
            </bem.FormModal__item>
          )}
            ...
            <bem.FormModal__item m="newForm-step2">
              {this.props.context !== 'replaceXLS' && (
                <bem.FormModal__item m="upload-note">
                  <label className="long">
                    {t(
                      'Project "##" has been created. Choose one of the options below to continue.'
                    ).replace('##', this.props.newFormAsset.name)}
                  </label>
                </bem.FormModal__item>
              )}
...
export class ProjectDownloads extends React.Component {
  ...
  langChange(e) {
    this.handleChange(e, 'lang');
  }
  fieldFromAllVersionsChange(e) {
    this.handleChange(e, 'fieldsFromAllVersions');
  }
  hierInLabelsChange(e) {
    this.handleChange(e, 'hierInLabels');
  }
  groupSepChange(e) {
    this.handleChange(e, 'groupSep');
  }
  handleSubmit(e) {
    e.preventDefault();
    this.setState({
                ...
                {[
                  <bem.FormModal__item key={'t'} m="export-type">
                    ...
                    >
                      ...
                      <option value="zip_legacy">
                        {t('Media Attachments (ZIP)')}
                      </option>
                      <option value="kml_legacy">
                        {t('GPS coordinates (KML)')}
                      </option>
                      <option value="analyser_legacy">
                        {t('Excel Analyser')}
                      </option>
                  ...
                  {this.state.exports.map((item, n) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <bem.FormView__label m="lang">
                          {item.data.lang === '_default'
                            ? t('Default')
                            : item.data.lang}
                        </bem.FormView__label>
                        <bem.FormView__label m="include-groups">
                          {item.data.hierarchy_in_labels === 'false'
                            ? t('No')
                            : t('Yes')}
                        </bem.FormView__label>
                        <bem.FormView__label m="multi-versioned">
                          ...
                          item.data.fields_from_all_versions === 'false'
                            ? t('No')
                            : t('Yes')}
                        </bem.FormView__label>
                        <bem.FormView__label m="action">
                          {item.status == 'complete' && (
                            <a
                              className="form-view__link form-view__link--export-download"
                              href={item.result}
                              data-tip={t('Download')}
                            >
                          ...
                          {item.status != 'error' &&
                            item.status != 'complete' && (
                              <span className="animate-processing">
                                {t('processing...')}
                              </span>
                            )}
                          <a
                            className="form-view__link form-view__link--export-delete"
...
let existingFormMixins = [Reflux.ListenerMixin, editableFormMixin];
