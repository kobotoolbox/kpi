...
var CollectionsGallery = React.createClass({
	...
	loadMoreAttachments(filter, index, page , callback){
		return dataInterface.paginateGalleryImages(this.props.uid, filter, index, page, this.state.defaultPageSize, callback).done((response)=>{
					...
					<bem.AssetGallery__grid>
						{this.state.assets.results.map(function(record, i) {
							if(typeof record.attachments.results == "undefined" || record.attachments.results.length === 0){
								console.log("No record attachments");
								console.log(record);
								return null;
							} else{
								...
								return (
									<div key={i}>
										...
										<LoadMoreAttachmentsBtn index={i} filter={this.state.filter.source} loadMore={this.loadMoreAttachments}/>
									</div>
								);
							}
						}.bind(this))}
...
var LoadMoreAttachmentsBtn = React.createClass({
  getInitialState: function() {
    return {
		page: 2,
		enabled: true
  ...
  handleClick: function() {
	...
	//TODO disable if the loadMore's response.attachments.next is null
  },
  render: function() {
	let btn = null;
