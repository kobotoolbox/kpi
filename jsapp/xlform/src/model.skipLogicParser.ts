...
const equalityCriterionPattern = /^\${(\w+)}\s*(!=|<=|>=|=|<|>)\s*((?:date\(\'\d{4}-\d{2}-\d{2}\'\))|(?:-?(?:\d+\.\d+|\.\d+|\d+.?))|(?:\'[^']+\'))/

// /\${(\w+)}\s*((?:=|!=)\s*(?:NULL|''))/i
const existenceCriterionPattern = /\${(\w+)}\s*((?:=|!=)\s*(?:NULL|''))/i
...
// /selected\(\$\{(\w+)\},\s*\'(\w+)\'\)/
const selectMultiplePattern = /selected\(\$\{(\w+)\},\s*\'(\w+)\'\)/

const skipLogicParser = $factory(
...
export default skipLogicParser
