...
export class Survey extends $surveyFragment.SurveyFragment {
}
...
module.exports = (function() {
 ...
 r = ref[i]
    ...
    linkUpChoiceLists() {
      ...
      var choiceKeys, choiceList, overlapping_choice_keys
      ...
      choiceKeys = this.choices.getListNames()
 ...
 choiceList = ref1[i1]
    ...
    insert_row(row, index) {
      ...
      var new_row, survey, rowlist, name_detail
 ...
 sd = ref5[i6]; if (!sd.get('value')) continue
      ...
      choicesCsvJson = (() => {
        var lists, rows, cols, choiceList, clAtts, clName, option
        lists = new $choices.ChoiceLists()
        this.forEachRow(function(r){
          var _getSubLists
          _getSubLists = function(item){
            var list
            if ('getList' in item) {
              list = item.getList()
 ...
 content = this.toCsvJson()[shtName]
  ...
  _is_csv = function(csv_repr){
    ...
    return indexOf.call(csv_repr, '\n') >= 0 && indexOf.call(csv_repr, ',') >= 0
  }
