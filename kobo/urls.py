...
class RawFilenameMultiPartParser(DRFMultiPartParser):
    ...
    def parse(self, stream, media_type=None, parser_context=None):
        ...
        try:
            ...
            parser = MultiPartParserWithRawFilenames(meta, stream, upload_handlers, encoding)
...
class MultiPartParserWithRawFilenames(DjangoMultiPartParser):
    ...
    def parse(self):
        ...
        for field_name, f in files.items():
            decoded = base64.urlsafe_b64decode(f.name.encode('ascii')).decode(
                'utf-8'
            )
...
class UploadEchoView(APIView):
    ...
    def post(self, request, format=None):
        ...
        return Response(
            {
                # repr() to show escape codes for weird characters
                'sanitized_filename__repr': repr(uploaded_file.name),
                'raw_filename__repr': repr(uploaded_file._raw_filename),
...
urlpatterns = [
    path('upload_echo/', UploadEchoView.as_view()),
    path(
        'api/v2/schema/',
