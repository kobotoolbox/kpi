...
class TestSimpleSubmission(TestCase):
    ...
    def test_accepts_well_formed_xml_with_parser_error_text_as_data(self):
        """
        A respondent legitimately typing text like "PCDATA invalid Char value"
        or "<parsererror" into a text question should not be rejected: the
        payload is well-formed XML with no actual invalid characters
        """
        self.xform1.require_auth = False
        self.xform1.save(update_fields=['require_auth'])
        ...
        valid_xml = (
            '<?xml version="1.0" ?>'
            '<yes_or_no id="yes_or_no">'
            '<yesno>I got a PCDATA invalid Char value error and also typed '
            '&lt;parsererror&gt; in my report</yesno>'
            '<meta><instanceID>uuid:{}</instanceID></meta>'
            '</yes_or_no>'
        ).format(str(uuid.uuid4()))

        error, instance = safe_create_instance(
            self.user.username, TempFileProxy(valid_xml), None, None
        )
        self.assertIsNone(error)
        self.assertIsNotNone(instance)

    def test_rejects_dom_parser_error_document_as_root(self):
        """
        If a client fails to serialise the real instance XML and instead submits
        the well-formed `<parsererror>` document produced by the browser's
        DOMParser/XMLSerializer, it should be rejected even though the document
        has no disallowed characters and is syntactically valid XML
        """
        firefox_parser_error_xml = (
            '<parsererror '
            'xmlns="http://www.mozilla.org/newlayout/xml/parsererror.xml" '
            'dir="ltr">XML Parsing Error: mismatched tag. Expected: '
        ...
        error, instance = safe_create_instance(
            self.user.username, TempFileProxy(firefox_parser_error_xml), None, None
        )
        text = 'unsupported or invisible characters'
        self.assertContains(error, text, status_code=400)

        chrome_parser_error_xml = (
            '<parsererror xmlns="http://www.w3.org/1999/xhtml" '
            'style="display: block; white-space: pre; border: 2px solid '
            '#c77; padding: 0 1em 0 1em; margin: 1em; background-color: '
            '#fdd; color: black">This page contains the following errors:'
            'error on line 1 at column 20: PCDATA invalid Char value 2\n'
            'Below is a rendering of the page up to the first error.'
            '</parsererror>'
        )

        error, instance = safe_create_instance(
            self.user.username, TempFileProxy(chrome_parser_error_xml), None, None
        )
        self.assertContains(error, text, status_code=400)
