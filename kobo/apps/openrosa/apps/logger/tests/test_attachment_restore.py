...
from kobo.apps.openrosa.apps.logger.tasks import restore_soft_deleted_attachments
...
class AttachmentRestorerTestCase(TestCase):
    ...
    def test_the_task_writes_nothing_unless_it_is_asked_to(self):
        """
        The admin hands `dry_run` straight through from the periodic task's
        keyword arguments, and the command spells the same thing
        `--no-dry-run`. Getting that default wrong would make a run meant to
        report restore instead.
        """

        _, attachment = self._submit_with_photo('first.jpg')
        self._kill(attachment)

        restore_soft_deleted_attachments(asset_uid=self.asset.uid)

        attachment.refresh_from_db()
        assert attachment.delete_status == AttachmentDeleteStatus.SOFT_DELETED

        restore_soft_deleted_attachments(asset_uid=self.asset.uid, dry_run=False)
