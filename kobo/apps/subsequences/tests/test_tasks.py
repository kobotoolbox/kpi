...
class TestPollRunExternalProcess(BaseTestCase):
    ...
    def _set_up_translation_action(self):
        translation_action_id = Action.AUTOMATIC_GOOGLE_TRANSLATION
        QuestionAdvancedFeature.objects.get_or_create(
            asset=self.asset,
            action=translation_action_id,
            question_xpath=self.question_xpath,
            defaults={'params': [{'language': 'en'}]},
        )
                ...
                'content': {
                    ...
                    self.question_xpath: {
                        Action.AUTOMATIC_GOOGLE_TRANSCRIPTION: {
                            '_dateCreated': '2026-04-29T10:00:00Z',
                            '_dateModified': '2026-04-29T10:00:00Z',
                            '_versions': [
                                {
                                    '_data': {
                                        'language': 'fr',
                                    ...
                                    '_dateCreated': '2026-04-29T10:00:00Z',
                                    '_dateAccepted': '2026-04-29T10:01:00Z',
        ...
        return translation_action_id

    @patch('kobo.apps.subsequences.actions.base.BaseAutomaticNLPAction.run_external_process')  # noqa: E501
    def test_poll_translation_raises_timeout_error_when_in_progress(self, mock_run):
        translation_action_id = self._set_up_translation_action()
        mock_run.return_value = {'status': 'in_progress', 'language': 'en'}
        ...
        with self.assertRaises(SubsequenceTimeoutError):
            poll_run_external_process(
                ...
                submission=self.submission,
                question_xpath=self.question_xpath,
                action_id=translation_action_id,
                action_data={'language': 'en', 'locale': None},
            )

    @patch('kobo.apps.subsequences.actions.base.BaseAutomaticNLPAction.run_external_process')  # noqa: E501
    def test_poll_translation_completes_successfully(self, mock_run):
        translation_action_id = self._set_up_translation_action()
        mock_run.return_value = {
            'status': 'complete',
            'value': 'Hello.',
        ...
        latest_version = (
            supplement_data[self.question_xpath][translation_action_id]['en'][
                '_versions'
            ][0]
        )
        self.assertEqual(latest_version['_data']['status'], 'complete')
        self.assertEqual(latest_version['_data']['value'], 'Hello.')
...
class TestPollRunExternalProcessFailure(BaseTestCase):
    ...
    def test_failure_handler_updates_translation(self):
        translation_action_id = Action.AUTOMATIC_GOOGLE_TRANSLATION
        QuestionAdvancedFeature.objects.get_or_create(
            asset=self.asset,
            action=translation_action_id,
                ...
                'content': {
                    ...
                    self.question_xpath: {
                        ...
                        translation_action_id: {
                            'en': {
                                '_dateCreated': '2026-04-29T10:02:00Z',
                                '_dateModified': '2026-04-29T10:02:00Z',
                                '_versions': [
                                    {
                                        '_data': {
                                            'language': 'en',
        ...
        poll_run_external_process_failure(
            sender=None,
            exception=Exception('Translation timed out'),
            **task_kwargs,
        )
        ...
        self.assertEqual(latest_version['_data']['status'], 'failed')
        self.assertEqual(latest_version['_data']['error'], 'Translation timed out')
