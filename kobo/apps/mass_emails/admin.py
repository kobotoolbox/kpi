...
class MassEmailConfigAdmin(admin.ModelAdmin):
    ...
    def enqueue_mass_emails(self, request, queryset):
        for config in queryset:
            ...
            # Create a job and records for each user asynchronously
            enqueue_mass_email_records.delay(config.id)

            self.message_user(
                request,
                f'Emails for {config.name} have been scheduled for the next day',
