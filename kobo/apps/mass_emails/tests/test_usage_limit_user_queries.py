...
class UsageLimitUserQueryTestCase(BaseServiceUsageTestCase):
    ...
    @data(
        ...
        ('submission', 'get_submissions_for_current_billing_period_by_user_id', False, True),
        ('seconds', 'get_nlp_usage_for_current_billing_period_by_user_id', False, True),
        ('characters', 'get_nlp_usage_for_current_billing_period_by_user_id', False, True),
    )
    @unpack
    def test_users_in_range_of_usage_limit_calls_correct_usage_method(
        self, usage_type, method_to_patch, include_storage_addons, include_onetime_addons,
    ):
