...
def get_users_within_range_of_usage_limit(
    ...
    :param usage_types: UsageType. List of usage types.
    ...
    user_ids = set()
    for usage_type in usage_types:
        for user_id, usage in usage_by_type_by_user[usage_type]:
            limit = limits_by_owner.get(user_id, {}).get(f'{usage_type}_limit') or inf
