...
def add_submission_to_group(sender, instance, created, **kwargs):
    ...
    if not created:
        return
    latest_group = SubmissionGroup.objects.filter(user_uid=instance.user_uid).order_by('-date_created').first()
    if latest_group is not None:
        time_limit = settings.get('ACCESS_LOG_SUBMISSION_GROUP_TIME_LIMIT_MINUTES')
...
def assign_self_to_group(sender, instance, created, **kwargs):
    # assign a submission group to itself. This makes serializaiton much easier
    if not created:
        return
