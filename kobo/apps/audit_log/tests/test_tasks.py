...
from kobo.apps.audit_log.tests.test_utils import create_submission_group_log, create_access_log_from_user_with_metadata
...
class AuditLogTasksTestCase(BaseTestCase):
    ...
    def test_spawn_new_deletion_task_correctly_identifies_expired_regular_logs(self):
        user = User.objects.get(username='someuser')
        three_days_ago = timezone.now() - timedelta(days=3)
        old_log = create_access_log_from_user_with_metadata(user=user, metadata_dict={'auth_type': 'token'})
        old_log.date_created = three_days_ago
        old_log.save()
        new_log = create_access_log_from_user_with_metadata(user=user, metadata_dict={'auth_type': 'token'})
        new_log.save()
        with patch('kobo.apps.audit_log.tasks.batch_delete_audit_logs_by_id.delay') as patched_spawned_task:
    ...
    def test_spawn_new_deletion_task_correctly_identifies_expired_submission_group_logs(self):
        user = User.objects.get(username='someuser')
        three_days_ago = timezone.now() - timedelta(days=3)
        two_days_ago = timezone.now() - timedelta(days=2)
        old_log = create_submission_group_log(user=user, latest_date=two_days_ago)
        old_log.date_created = three_days_ago
        old_log.save()
        # mock up a submission group that was created before the expiration cutoff but whose latest entry is more recent
        newer_log = create_submission_group_log(user=user, latest_date=timezone.now())
        newer_log.date_created = two_days_ago
        newer_log.save()
        # both date_created and latest entry are after the expiration cut off
        newest_log = create_submission_group_log(user=user, latest_date=timezone.now())
        newest_log.save()
        newest_log.save()
        with patch('kobo.apps.audit_log.tasks.batch_delete_audit_logs_by_id.delay') as patched_spawned_task:
            spawn_access_log_cleaning_tasks()
    ...
    def test_spawn_new_deletion_tasks_batches_expired_logs(self):
        ...
        old_logs = [create_access_log_from_user_with_metadata(user=user, metadata_dict={'auth_type': 'token'}) for i in range(5)]
