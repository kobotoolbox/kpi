...
class TestOneTimeAuthentication(BaseTestCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.user = User.objects.create_user(username='test', password='test', email='test@example.com')
        cls.user.is_superuser = True
        cls.user.save()
        profile = UserProfile.objects.create(user=cls.user, validated_password=True)
        profile.save()
    ...
    def setUp(self):
        ...
        get_mfa_model().objects.filter(user=TestOneTimeAuthentication.user).delete()
    ...
    @data(
        ...
        ('Token', 'kpi.authentication.DRFTokenAuthentication.authenticate', 'data-list'),
        ('Basic', 'kpi.authentication.DRFBasicAuthentication.authenticate', 'api_v2:audit-log-list'),
        ('OAuth2', 'kpi.authentication.OPOAuth2Authentication.authenticate', 'data-list'),
        ('Https Basic', 'kobo.apps.openrosa.libs.authentication.BasicAuthentication.authenticate', 'data-list'),
    )
    @unpack
    def test_one_time_auths_create_logs(self, expected_type, method_to_mock, url):
        """
        ...
        """
        with patch(method_to_mock, return_value=(TestOneTimeAuthentication.user, 'something')):
    ...
    def test_digest_auth_creates_logs(self):
        ...
        with patch('kpi.authentication.HttpDigestAuthenticator.authenticate', return_value=True,
    ...
    def test_digest_auth_for_submission(self):
        ...
        with patch('kpi.authentication.HttpDigestAuthenticator.authenticate', return_value=True,
            ...
            with patch('kobo.apps.openrosa.apps.api.viewsets.xform_submission_api.XFormSubmissionApi.create',
