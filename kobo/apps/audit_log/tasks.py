...
def spawn_access_log_cleaning_tasks():
    ...
    group_submission_logs = Q(
        log_type=AuditType.ACCESS,
        metadata__auth_type=SUBMISSION_GROUP_AUTH_TYPE,
    )
    expired_group_submission_logs = group_submission_logs & Q(
        metadata__latest_entry__lt=expiration_date
    )
    # non-submission-group logs expire when they were created > ACCESS_LOG_LIFESPAN days ago
    expired_other_logs = (
        Q(log_type=AuditType.ACCESS, date_created__lt=expiration_date)
        & ~group_submission_logs
    )
    expired_logs = (
