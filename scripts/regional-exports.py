...
def run(*args):
    if len(args) < 2:
        print_and_exit('Export type and username must be included in --script-args')

    export_type = args[0]
    if export_type not in ['p', 'u']:
        print_and_exit('Specify projects (p) or users (u) export in first argument of --script-args')

    username = args[1]
    try:
        user = User.objects.get(username=username)
    except User.DoesNotExist:
        print_and_exit('User does not exist')
    ...
    if not views_for_user:
        print_and_exit('User does not have a view assigned to them')
    ...
    if view is None:
        ...
        print_and_exit(f'User does not have view {view} assigned to them')
    ...
    filename = get_filename(export_type)
