...
for container_name in "${!container_ports[@]}"; do
    ...
    if [[ "${web_server^^}" != 'UWSGI' ]] ; then
        ...
        cat ${KOBO_DOCKER_SCRIPTS_DIR}/templates/proxy_pass.conf.tmpl \
            ...
            > ${INCLUDES_DIR}/${container_name_var}_proxy_pass.conf

        # Prepare to include the generated `proxy_pass` config. and no `uwsgi_pass` config.
        include_proxy_pass="include ${INCLUDES_DIR}/${container_name_var}_proxy_pass.conf;"
        include_uwsgi_pass=''
    else
        ...
        cat ${KOBO_DOCKER_SCRIPTS_DIR}/templates/uwsgi_pass.conf.tmpl \
            ...
            > ${INCLUDES_DIR}/${container_name_var}_uwsgi_pass.conf

        # Prepare to include the generated `uwsgi_pass` config. and no `proxy_pass` config.
        include_proxy_pass=''
        include_uwsgi_pass="include ${INCLUDES_DIR}/${container_name_var}_uwsgi_pass.conf;"
    fi
    include_proxy_pass_varname="${container_name_var}_include_proxy_pass"
    export ${include_proxy_pass_varname}="${include_proxy_pass}"
    include_uwsgi_pass_varname="${container_name_var}_include_uwsgi_pass"
