...
def build_url_type(viewname: str, **kwargs) -> dict:
    ...
    """
    Due to the life cycle of DRF, we have no choice but to build the examples as belong instead
    of using a reverse_lazy like we did before. This util is loaded before the urls and models
    which mean the cycle will enter an infinite loop where it tries to get the url, then the
    model and so on.
    """
    ...
    urls_pattern_mapping = {
        ...
        'assetsnapshot-xml-with-disclaimer': '/api/v2/asset_snapshots/{uid}/xml_with_disclaimer/',
        ...
        'attachment-detail': '/api/v2/assets/{parent_lookup_asset}/data/{parent_lookup_data}/attachments/{pk}',
        'attachment-thumb': '/api/v2/assets/{parent_lookup_asset}/data/{parent_lookup_data}/attachments/{pk}/{suffix}/',
        'paired-data-detail': '/api/v2/assets/{parent_lookup_asset}/paired-data/{paired_data_uid}',
    }
