...
# given serializer.
def open_api_generic_response(http_code: str, given_serializer: Serializer) -> {}:
    return {
        http_code: OpenApiResponse(
            response=given_serializer
        )
    }

# Returns an OpenApiResponse with the given serializer and a 200 http code
def open_api_200_ok_response(given_serializer: Serializer) -> {}:
...
def open_api_201_created_response(given_serializer: Serializer) -> {}:
...
def open_api_202_accepted_response(given_serializer: Serializer) -> {}:
...
def open_api_204_empty_response(given_serializer: Serializer) -> {}:
