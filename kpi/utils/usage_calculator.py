...
class BillingAndUsageCalculator:
    ...
    def calculate_usage_batch(
        self, organizations: list, billing_dates: dict
    ) -> dict:
        org_map = {}
        ...
        submission_map = self._get_submission_usage_batch(
            user_ids,
            {
                v['effective_user_id']: v['billing_dates']
                for v in org_map.values()
            },
        )
        ...
        for org_id, info in org_map.items():
            ...
            result[org_id] = {
                ...
                'submission_counts_all_time':
                    submission_map.get(uid, {}).get('all_time', 0),
                'current_period_submissions':
                    submission_map.get(uid, {}).get('current_period', 0),
