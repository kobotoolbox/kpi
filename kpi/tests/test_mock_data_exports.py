...
class MockDataExports(MockDataExportsBase):
    ...
    def run_csv_export_test(
        ...
        `export_options`: (optional) a list of extra options for
                          `SubmissionsExportTask.data`. Do not include `source` or `type`
    ...
    def run_xls_export_test(
        ...
        `expected_rows`: a list of strings *without* trailing newlines whose
                          UTF-8 encoded representation should match the export
                          result
        `export_options`: a list of extra options for `SubmissionsExportTask.data`. Do not
    ...
    def test_remove_excess_exports(self):
        ...
        export_task.save()
        export_task.run()
        self.assertEqual(export_task.status, SubmissionsExportTask.COMPLETE)
        ...
        for _ in range(excess_count):
            ...
            export_task.data = task_data
            export_task.save()
        created_export_tasks = SubmissionsExportTask.objects.filter(
        ...
        self.assertListEqual(  # assertSequenceEqual isn't working...
            ...
            list(
                SubmissionsExportTask.objects.filter(
                    user=self.user, data__source=task_data['source']
    ...
    def test_log_and_mark_stuck_exports_as_errored(self):
        ...
        for status in (SubmissionsExportTask.CREATED, SubmissionsExportTask.PROCESSING):
            export_task = SubmissionsExportTask()
            export_task.user = self.user
            export_task.data = task_data
        ...
        self.assertSequenceEqual(
            [SubmissionsExportTask.CREATED, SubmissionsExportTask.PROCESSING],
            SubmissionsExportTask.objects.filter(
                user=self.user, data__source=task_data['source']
        ...
        self.assertSequenceEqual(
            [SubmissionsExportTask.ERROR, SubmissionsExportTask.ERROR, SubmissionsExportTask.COMPLETE],
            SubmissionsExportTask.objects.filter(
                user=self.user, data__source=task_data['source']
            ).order_by('pk').values_list('status', flat=True),
        )
    ...
    def test_export_long_form_title(self):
        ...
        assert len(what_a_title) > SubmissionsExportTask.MAXIMUM_FILENAME_LENGTH
        ...
        assert (
            len(os.path.basename(export_task.result.name)) ==
                SubmissionsExportTask.MAXIMUM_FILENAME_LENGTH
        )
