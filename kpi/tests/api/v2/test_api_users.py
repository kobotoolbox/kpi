...
class UserListTests(BaseTestCase):
    ...
    def test_user_list_forbidden_non_superuser(self):
        ...
        url = reverse(self._get_endpoint('user-kpi-v1-list'))
        response = self.client.get(url, format='json')
    ...
    def test_user_page_succeeds(self):
        ...
        """
        username = 'admin'
        url = reverse(self._get_endpoint('user-kpi-v1-detail'), args=[username])
        response = self.client.get(url, format='json')
    ...
    def test_invalid_user_fails(self):
        ...
        invalid user
        """
        url = reverse(self._get_endpoint('user-kpi-v1-detail'), args=['nonexistentuser'])
        response = self.client.get(url, format='json')
